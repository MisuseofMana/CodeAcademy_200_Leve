...
The actor table has a one (and only one) relationship from the film_actor table. The film_actor table has a One or Many relationship to the actor table.

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?
A blue lined filled diamond represents a "simple attribute" one with no key.

3. How many foreign keys does the "payments" table have? How can you tell?
Red icons in the the ER Diagram from mySqlWorkbench represent foreign keys. In this case there are three: customer_id from the customer table, staff_id from the staff table, and rental_id from the rental table.
